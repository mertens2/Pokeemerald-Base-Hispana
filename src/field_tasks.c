...
static void AshGrassPerStepCallback(u8 taskId)
{
    ...
    if (MetatileBehavior_IsAshGrass(MapGridGetMetatileBehaviorAt(x, y)))
    {
        ...
        if (MapGridGetMetatileIdAt(x, y) == METATILE_FallarborNew_AshGrass)
            StartAshFieldEffect(x, y, METATILE_FallarborNew_NormalGrass, 4);
		if (MapGridGetMetatileIdAt(x, y) == METATILE_LavaridgeNew_AshGrass)
			StartAshFieldEffect(x, y, METATILE_LavaridgeNew_CleanGrass, 4);

        // Try to gather ash
        if (CheckBagHasItem(ITEM_SOOT_SACK, 1))
        {
            ashGatherCount = GetVarPointer(VAR_ASH_GATHER_COUNT);
            if (*ashGatherCount < 50000)
                (*ashGatherCount)+=5;
        }
    }
}
