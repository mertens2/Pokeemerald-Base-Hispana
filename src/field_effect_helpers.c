...
void UpdateTallGrassFieldEffect(struct Sprite *sprite)
{
	...
	// else if (TryGetObjectEventIdByLocalIdAndMap(localId, mapNum, mapGroup, &objectEventId)
     // || !MetatileBehavior_IsDarkGrass(metatileBehavior)
     // || (sprite->sObjectMoved && sprite->animEnded))
    // {
        // FieldEffectStop(sprite, FLDEFF_DARK_GRASS);
    // }
    else
    {
...
void UpdateDarkGrassFieldEffect(struct Sprite *sprite)
{
    u8 metatileBehavior;
    u8 localId;
    u8 objectEventId;
    ...
    for (i = 0; i < MAX_SPRITES; i ++)
    {
        if (gSprites[i].inUse)
        {
            ...
            if (sprite->callback == UpdateTallGrassFieldEffect
                ...
                && mapGroup == sprite->sMapGroup)
                return i;
			else if (sprite->callback == UpdateDarkGrassFieldEffect
                && (x == sprite->sX && y == sprite->sY)
                && localId == (u8)(sprite->sLocalId)
                && mapNum == (sprite->sMapNum & 0xFF)
                && mapGroup == sprite->sMapGroup)
...
u32 FldEff_DarkGrass(void)
{
    u8 spriteId;
    s16 x = gFieldEffectArguments[0];
    s16 y = gFieldEffectArguments[1];
    SetSpritePosToOffsetMapCoords(&x, &y, 8, 8);
    spriteId = CreateSpriteAtEnd(gFieldEffectObjectTemplatePointers[FLDEFFOBJ_DARK_GRASS], x, y, 0);
    if (spriteId != MAX_SPRITES)
    {
        struct Sprite *sprite = &gSprites[spriteId];
        sprite->coordOffsetEnabled = TRUE;
